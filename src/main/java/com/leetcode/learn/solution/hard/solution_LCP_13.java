 ...
 */
public class solution_LCP_13 {
    public static void main(String[] args) {
        String[] dungeon = new String[]{
            "S#O", "M..", "M.T"
        };
        new solution_LCP_13().minimalSteps(dungeon);
    }
    ...
    int count = 0;
    public int minimalSteps_1(String[] maze) {
        if(maze.length == 0)
            return -1;
        m = maze.length;
        n = maze[0].length();
        int si = 0,sj = 0,ti = 0,tj = 0;
        Set<int[]> ms = new HashSet();
        ...
        for(int i = 0;i < m;i++){
            for(int j = 0;j < n;j++){
                if(maze[i].charAt(j) == 'S'){
                    ...
                    ms.add(new int[]{i + 1,j + 1});
                }else if(maze[i].charAt(j) == '#'){
                    //墙的话就没有，就过不去
                    continue;
                }
                map.put(i * n + (j + 1),maze[i].charAt(j));
            }
        }

        //先从起点到机关处
        for(int[] ml:ms){
            getMinc(new int[]{si,sj},ml, map);
        }
     ...
     * @return
     */
    public int getMinc(int[] s, int[] t,HashMap map){
        //使用set防止重复
        ...
        while(queue.size() != 0){
            leval += 1;
            Set<int[]> next_queue = new HashSet<>();
        }
        return 0;
    }
}
